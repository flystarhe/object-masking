...
class MyConfig(configlib.Config):
...
def get_model(mode, config, model_dir, init_with="coco"):
    ...
    """
    model = modellib.MaskRCNN(mode, config, model_dir)
...
def dataset_display_masks(dataset, image_ids=None, limit=4):
    ...
    for image_id in image_ids:
        ...
        visualize.display_top_masks(image, mask, class_ids, dataset.class_names, limit)
...
def dataset_display_instances(config, dataset, image_ids=None, augmentation=None):
    ...
    for image_id in image_ids:
        image, image_meta, gt_class_id, gt_bbox, gt_mask =\
            modellib.load_image_gt(dataset, config, image_id, augmentation=augmentation, use_mini_mask=False)
        print(dataset.image_reference(image_id))
...
def detect_display_masks(image, r, class_names, limit=4):
    if isinstance(image, str):
        ...
        if image.shape[-1] == 4:
            image = image[..., :3]
    visualize.display_top_masks(image, r["masks"], r["class_ids"], class_names, limit)
...
def detect_display_instances(image, r, class_names):
    if isinstance(image, str):
        ...
        if image.shape[-1] == 4:
            image = image[..., :3]
    visualize.display_instances(image, r["rois"], r["masks"], r["class_ids"], class_names, scores=r["scores"])
