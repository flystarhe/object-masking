...
def detection_bright_spots(filename, bright=200, cfg={}):
    ...
    kmin, kmax, rate = cfg.get('object_area', (9, 100, 0.40))
    ...
    grad = np.logical_or(grad_x>threshold, grad_y>threshold)
    grad = np.logical_and(grad, gray>bright).astype(np.uint8)
    resx = grad.copy()

    grad = subs_label(grad, 9).astype('uint8')
...
cfg = {'debug': True,
       ...
       'blur_ksize': 3,
       'dilate_ksize': 3,
       'object_area': (9, 300, 0.40)}
data_dir = '/data2/datasets/slyx/tmps/mb1_base_jun'
save_dir = '/data2/tmps/mb1_stage3/v3_{}_{}_{}_9_300_040'.format(cfg['threshold'], cfg['blur_ksize'], cfg['dilate_ksize'])
if os.path.isdir(save_dir):
    shutil.rmtree(save_dir)
os.system('mkdir -m 777 -p {}'.format(save_dir))
filepaths = [os.path.join(data_dir, filename) for filename in os.listdir(data_dir)]
for filepath in filepaths:
    logs1, bbox1, grad1 = detection_bright_spots(filepath, 70, cfg)
    logs2, bbox2, grad2 = detection_bright_spots(filepath, 210, cfg)
    logs = logs1 + logs2
    bbox = bbox1
    grad = grad1
    save_image(filepath, todo_logs(logs.copy()), bbox, grad, save_dir=save_dir)